...
|--------|---------------|--------------|
| `adapters/hook-adapter/src/installer.rs` | 安装/卸载/状态/预览 hooks 到 ~/.claude/settings.json (CC) 和 ~/.codex/config.toml (Codex) | serde_json, toml |
| `adapters/hook-adapter/src/hook.rs` | 从 stdin 读取 hook JSON，POST 到事件服务器（含进程检测 + agent 来源识别） | serde_json, ureq, sysinfo |
...
| `apps/desktop/src-tauri/src/hooks.rs` | Hook 配置管理：build, ensure, unregister, get_status (CC + Codex) | serde_json, toml |
...
agentpulse-hook ──HTTP POST──→ event_server.rs
...
- **Hook Adapter ↔ Rust**: 仅通过 HTTP (POST /api/events)，Rust 二进制通过 stdin → HTTP 转发
